...
// Fetch all payment details for a specific email ID
app.get('/fetch-user-payment', async (req, res) => {
  const email = req.query.email; // assume you'll send email as a query parameter
  console.log(email);
  if (!email) {
    return res.status(400).json({ message: 'Email is required!' });
  ...
  try {
    const pay = await Payment.find({ email: email });
    if (pay.length === 0) {
      return res.status(404).json({ message: 'No payments found for the provided email!' });
    }
    res.json(pay);
    console.log(pay);
  } catch (error) {
